  ...
  <div class="card col-5" style="width: 20rem">
    ...
    <div class="card-body">
      ...
      <a href="{{ route('wishlist.add', $fragance->getId()) }}" class="btn btn-outline-success btn-block mt-2 col-12"
        >{{ __("home.add-wish") }} <i class="fa far fas fa-heart"></i
      ></a>
      @else
      <form action="{{ route('wishlist.delete', $fragance->getId()) }}" method="POST">
        @csrf @method('DELETE')
        <button type="submit" class="btn btn-outline-success btn-block mt-2 col-25">Delete Wish <i class="fa far fas fa-heart"></i></button>
      </form>
      @endif @if (is_null($item))
      <form action="{{ route('item.delete', $fragance->getId()) }}" method="post" class="row justify-content-around">
        @csrf
        <button type="submit" class="btn btn-outline-primary btn-block col-7 mt-2">
          {{ __("home.add") }}
          <i class="fa fas fa-shopping-cart"></i>
        </button>
        <input type="number" class="btn btn-outline-primary btn-block col-3" name="quantity" value="1" min="1" max="99" />
      </form>
      @else
      <form action="{{ route('item.delete', $fragance->getId()) }}" method="POST">
        @csrf @method('DELETE')
        <button type="submit" class="btn btn-outline-primary btn-block col-15 mt-3">
...
<div class="mt-5 list-group-item text-center text-uppercase m-5">
  {{ __("home.api-notw") }}/div> @else
  ...
  @endif @if (auth()->user()) @section('user', auth()->user()->getName(),) @endif @endsection
</div>
